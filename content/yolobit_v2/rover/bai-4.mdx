---
title: "7. Bài 4: Trở thành cảnh sát"
---

# 7. Bài 4: Trở thành cảnh sát

*Rover có một đôi mắt cực kì nhạy bén với rất nhiều chức năng và công dụng.Trong đó, đo đạc là chức năng hữu ích nhất mà Rover thường dùng trong thành phố. Với đôi mắt đó, bạn có thể đo lường khoảng cách một cách dễ dàng mà không cần các thước đo thủ công.*

## Mục tiêu
1. Làm quen với khối lệnh âm nhạc
2. Biết cách điều khiển đèn màu trên Rover

## Bộ phận điện tử
- Loa: Bộ phận phát âm thanh nằm ở mặt sau Yolo:Bit
![bai 4.1](/images/yolobit_v2/rover/images/bai_4.1.png)


- Đèn LED RGB có thể hiển thị nhiều màu sắc khác nhau. Rover được trang bị 6 chiếc đèn này
![bai 4.2](/images/yolobit_v2/rover/images/bai_4.2.png)


## Giới thiệu khối lệnh
- Khối lệnh đèn RGB
![bai 4.3](/images/yolobit_v2/rover/images/bai_4.3.png)


- Khối lệnh âm nhạc
![bai 4.4](/images/yolobit_v2/rover/images/bai_4.4.png)


## Viết chương trình
1. Phát bài nhạc khi bắt đầu chương trình và cho Rover di chuyển
![bai 4.5](/images/yolobit_v2/rover/images/bai_4.5.png)


2. Cho dãy LED RGB sáng xanh đỏ luân phiên
![bai 4.6](/images/yolobit_v2/rover/images/bai_4.6.png)


## Chương trình mẫu
- Trở thành cảnh sát: [Tại đây ](https://app.ohstem.vn/#!/share/yolobit/2BeV3NydHQBWHOMNWowpDh7aRQ4)
![bai 4.7](/images/yolobit_v2/rover/images/bai_4.7.png)
